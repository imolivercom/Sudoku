const gridEl=document.getElementById('grid');const winEl=document.getElementById('win');const newBtn=document.getElementById('newGame');
...
function build(){const{puzzle,solved}=generate();solution=solved;gridEl.innerHTML='';
 for(let r=0;r<9;r++){for(let c=0;c<9;c++){const cell=document.createElement('div');cell.className='cell';
  ...
  const v=puzzle[r][c];if(v){cell.textContent=v;cell.classList.add('prefilled');}else{cell.onclick=()=>sel(cell);}gridEl.appendChild(cell);}}}
function sel(cell){if(popup)popup.remove();const wrap=document.querySelector('.board-wrapper');const R=wrap.getBoundingClientRect();
 const size=Math.min(R.width*0.35,160);popup=document.createElement('div');popup.className='popup';popup.style.width=popup.style.height=size+'px';
 ...
 wrap.appendChild(popup);const cRect=cell.getBoundingClientRect();let l=cRect.left-R.left,t=cRect.top-R.top;if(l+size>R.width)l=R.width-size;if(t+size>R.height)t=R.height-size;if(l<0)l=0;if(t<0)t=0;popup.style.left=l+'px';popup.style.top=t+'px';}
function set(cell,val){const r=+cell.dataset.row,c=+cell.dataset.col;cell.textContent=val;const ok=val==solution[r][c];
 ...
 if(ok&&[...gridEl.children].every(el=>el.classList.contains('prefilled')||el.classList.contains('correct'))){winEl.classList.add('show');submit().finally(()=>setTimeout(()=>{winEl.classList.remove('show');build();},1500));}}
async function submit(){if(!jwt)return;await fetch('/api/score',{method:'POST',headers:{Authorization:'Bearer '+jwt}}).catch(console.error);}
newBtn.onclick=()=>build();
window.initGame = build;
